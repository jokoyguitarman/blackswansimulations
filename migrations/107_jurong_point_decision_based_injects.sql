-- Add Decision-Based Injects to Jurong Point Bombing Scenario
-- These injects trigger based on player decisions, not time
-- Run this AFTER demo/seed_jurong_point_scenario.sql has been executed
...
BEGIN
  ...
  IF scenario_uuid IS NULL THEN
    RAISE NOTICE '107: Jurong Point scenario not found; skipping. Run demo/seed_jurong_point_scenario.sql first.';
    RETURN;
  END IF;
  ...
  SELECT
    ...
    'Despite your evacuation order to group everyone together, many people refuse to be grouped with Malay families due to fear from the viral misinformation. Protests break out at the evacuation zone. Some attendees are creating their own exit routes through the mall, causing chaos. A Malay volunteer assigned to evacuation is confronted by frightened residents. The evacuation is now stalled.',
    'critical',
    '[]'::jsonb,
    'universal',
    NULL,
  ...
  SELECT
    ...
    'Frightened attendees are bypassing your evacuation plan and creating their own exit routes through stairwells and corridors. This uncontrolled movement is creating new bottlenecks and potential stampede risks across multiple floors. The suicide attacker (if still on site) could exploit these chaotic exit points. You''ve lost control of the evacuation flow.',
    'high',
    '[]'::jsonb,
    'universal',
    NULL,
  ...
  SELECT
    ...
    'As you delay the evacuation to investigate, public pressure is mounting. Families are demanding to know why people aren''t being moved to safety. Online commentators are criticizing the delay. The situation inside the mall is becoming more volatile as people feel trapped across multiple floors.',
    'high',
    '[]'::jsonb,
    'universal',
    NULL,
  ...
  SELECT
    ...
    'By keeping people on site longer, you''ve created a larger target for the potential suicide attacker. Intelligence suggests that maximum crowd density at stairwells or exits is exactly what the attacker is waiting for. The delay may be playing into the attacker''s hands.',
    'critical',
    '[]'::jsonb,
    'universal',
    NULL,
  ...
  SELECT
    ...
    'While you focus on triage, the evacuation teams are operating without proper coordination. Stairwells and exit routes are becoming more congested. The lack of communication between triage, evacuation, and fire is creating bottlenecks and confusion across multiple floors.',
    'high',
    '[]'::jsonb,
    'universal',
    NULL,
  ...
  SELECT
    ...
    'The suspicious individual near Stairwell B is now moving closer to a crowded evacuation route. Volunteers report the person is acting erratically and appears to be waiting for something. You may have missed a critical security threat by dismissing the initial report.',
    'critical',
    '[]'::jsonb,
    'universal',
    NULL,
  ...
  SELECT
    ...
    'The combination of evacuating everyone together (without addressing fears) and failing to counter the misinformation has created a perfect storm. People are refusing to evacuate, creating their own exit routes through the mall, and the false narrative is now being treated as fact. The situation is spiraling out of control. Multiple teams need to coordinate an emergency response.',
    'critical',
    '[]'::jsonb,
    'universal',
    NULL,
  ...
  RAISE NOTICE 'Decision-based injects processed for Jurong Point Bombing scenario!';
