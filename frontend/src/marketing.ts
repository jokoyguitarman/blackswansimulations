...
if (storyboard) {
  ...
  // Counting the numbers up rather than swapping them is what makes the
  // deterioration feel like it is happening to the reader.
  const frames = new Map<HTMLElement, number>();
  const tweenTo = (el: HTMLElement | null, target: number, format: (n: number) => string) => {
    if (!el) return;
