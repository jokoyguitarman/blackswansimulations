import type { CasualtyVictim, CasualtyPin, TriageTag } from './types';
...
export function generateSingleCasualty(
  ...
  const preset = band.presets[Math.floor(Math.random() * band.presets.length)];

  return {
    id: `cas-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
    pos: { ...pos },
    description: preset.description,
    trueTag: preset.trueTag,
    currentTag: preset.trueTag,
    observableSigns: { ...preset.observableSigns },
    imageUrl: null,
    imageGenerating: false,
    autoGenerated: false,
