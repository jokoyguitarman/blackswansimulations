...
export interface CasualtyPin {
  id: string;
  pos: Vec2;
  description: string;
  trueTag: TriageTag;
  currentTag: TriageTag;
  observableSigns: {
    breathing: string;
    pulse: string;
    consciousness: string;
...
// ── Game zones (hot/warm/cold operational zones) ──────────────────────
export type GameZoneType = 'hot' | 'warm' | 'cold';
...
export const GAME_ZONE_DEFS: Record<
  GameZoneType,
  { label: string; color: string; multiplier: number }
> = {
  hot: { label: 'Hot Zone', color: '#ef4444', multiplier: 1 },
  warm: { label: 'Warm Zone', color: '#f97316', multiplier: 2 },
  cold: { label: 'Cold Zone', color: '#eab308', multiplier: 3 },
};
