...
| Log line                                                                                                                  | Where                                                                                                             | Effect                                                                                                                                                              | Status                                                                                                                                                                      |
| ------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `session_events insert failed — Could not find the 'user_id' column of 'session_events'` (`eventType: decision_detected`) | `services/decisions/sessionEventEmitter.ts`                                                                       | Organic-decision detections were never recorded in the trainer timeline                                                                                             | **Fixed 21 Sep** (runtime agent, touch point in the generator module): inserts use `actor_id`, the actual column                                                            |
| `cascade inject insert failed — invalid input syntax for type integer: "57.34045"`                                        | `services/decisions/decisionContext.ts`, `engineTicker.ts`, `decisionPlannerCore.ts`, `decisionCascadeService.ts` | Every cascade reaction inject (and `decision_knowledge` / `decision_events` rows) was dropped: elapsed minutes were fractional and the minute columns are `INTEGER` | **Fixed 21 Sep**: `elapsedMinutesOf()` and the ticker return whole minutes (`Math.floor`); inject minute writes also `Math.round` defensively                               |
| `Ambient AI call failed — Unterminated string in JSON at position ~4500` (every few minutes)                              | `services/ambientContentService.ts` `generateFacebookAmbientPosts` (also thread continuation)                     | Model output was cut at 1,000 tokens → JSON parse failed → Fakebook ambient posts skipped that tick                                                                 | **Fixed 21 Sep**: 2,500-token budget, 3–4 posts, image prompts ≤ 25 words; thread continuation raised to 1,500                                                              |
| `Failed to send invitation email — 535 BadCredentials` (Gmail SMTP)                                                       | `services/emailService.ts` via `SMTP_USER` / `SMTP_PASS` (default host `smtp.gmail.com:587`)                      | Session invitations are not delivered (participant rows are still created; joining by link works)                                                                   | **Product owner**: Google rejects the stored credential — generate a new Gmail App Password for `SMTP_USER`, update `SMTP_PASS` in the Render service environment, redeploy |
| `Grok video start API error — team … has used all available credits`                                                      | xAI video generation via `XAI_API_KEY`                                                                            | Generated video assets fail until credits are added; posts fall back to text/image                                                                                  | **Product owner**: top up credits / raise the xAI spending limit, or leave as is if video is not needed                                                                     |
