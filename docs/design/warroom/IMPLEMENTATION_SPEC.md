...
**Still open (unchanged from §11):** draft discard from the ledger; a read-only Preview distinct from the editor; the field-ops Scene editor / Location / Research / Compile step bodies; Dashboard and AAR pages.

### 12.1 Second pass (2026-09-20, evening) — commits `c103583`, `951cc01`

Extended the design to the surfaces the trainer meets after Setup:

| Surface                   | File                                                             | What changed                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Wizard · Building         | `SocialCrisisWizard.tsx` `renderBuilding`                        | four-stage rail (queued / running / done / failed) with progress bar, live storyline log, retry; **new:** a "Build the scenario" action when a draft is resumed mid-build (generation runs in the browser; before this the resumed step showed a spinner forever)                                                                                                                                                                                                         |
| Wizard · Review & compile | `renderStep7`                                                    | narrative band with the scenario's art, eight generated-count tiles, "Run by the AI" cards (AI offices + pressure groups with spokesperson and statement counts), team cards grouped by organisation, SOP steps, folding read-through (injects with origin badges, contacts, personas, fact sheet, objectives, gates, pages); Compile lives in the sticky bar and at the foot                                                                                             |
| Wizard · Success          | `renderStep7`                                                    | illustrated card with check tile, title, id, KPIs; actions: View in the library · Create a session (preselected via `?create=`) · Modify & recompile; compile log folded                                                                                                                                                                                                                                                                                                  |
| Sessions page             | `Sessions.tsx`                                                   | hero with nav + live / scheduled / completed / participants counts and Create session; status toolbar with search; Live now / Scheduled / Completed groups as art rows (T+ clock on live, date otherwise) with Start / Open lobby / Join / View AAR / Spectate                                                                                                                                                                                                            |
| Create session            | `Forms/CreateSessionModal.tsx`                                   | dark bar with credits pill, searchable poster-row scenario picker (preselect honoured), schedule + instructions, "what happens next", sticky footer; `scenarios` prop widened to `CreateSessionScenario` (additive)                                                                                                                                                                                                                                                       |
| Session lobby             | `SessionView.tsx` (scheduled branch), `Session/SessionLobby.tsx` | hero with scenario art / title / participants / AI teammates / scheduled start; readiness card (trainer: progress bar + rows + Start; participant: one ready action); team cards with icons, monograms, bot badges, "your team"; briefing + participant management framed. `AITeammatesPanel`, `JoinLinkPanel`, `ParticipantManagement`, `BriefingView` untouched inside. `Session` type in `SessionView` gained optional `sim_mode`, `scenarios.category`, `user.is_bot` |

**Follow-ups logged from review (not yet done):**

1. **Team assignments modal** (`Teams/TeamAssignmentModal.tsx`) — the header row and the radio grid use different column widths, so the circles drift from their team labels and the table scrolls horizontally. Rebuild as one CSS grid: team columns as the sticky header (icon + name + cap), player rows with monogram + bot badge, radio cells aligned under the header, unassigned count and Auto-balance in a `wr-ctabar`. No horizontal scroll under ~8 teams; wrap into two rows beyond.
2. **Join link panel** (`Session/JoinLinkPanel.tsx`) — restyle to a `wr-node` with the link in a mono field and Copy / Regenerate / Disable as `wr-btn`s; expiry as a pill.
3. **AI teammates panel** (`Session/AITeammatesPanel.tsx`, other agent's file) — visually align with the lobby cards once their work settles (tile + team icon, bots as `wr-mono ai`, intellect slider in a `wr-node`).
4. **Participant management** (`Session/ParticipantManagement.tsx`, other agent's file) — same.
5. **Live session header** (`SessionView.tsx` in-progress branch, line ~1600) — the terminal-style header with Player / Teams badges is unchanged; give it the `wr-bar` treatment.
6. **Page assignment modal** (`Teams/PageAssignmentModal.tsx`) — same treatment as the team assignments modal.
