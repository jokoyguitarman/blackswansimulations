...
**Sector standards and scoring:** C2E uses ICS/WHO/ICRC-UNOCHA sector standards to calibrate robustness, environmental consistency, and relevance/detail in decision evaluation. Decisions that specify ratios (e.g. marshal-to-evacuee, triage staff-to-critical), capacities (assembly 125%, triage zone ~50), protocols (START), or media discipline (designated spokesperson, briefing frequency) score more favourably; vague or absent specifications are penalised. Baseline escalation factors that reference these standards (e.g. overcrowding at assembly, no spokesperson) seed session escalation context. See [C2E_SECTOR_STANDARDS.md](C2E_SECTOR_STANDARDS.md).
