...
export interface SocialInjectDeliveryConfig {
  ...
  country?: string;
  /** Stable cross-reference for template gating (inject_published:<key>); reserved §7A. */
  inject_key?: string;
  /** Executive decision this template belongs to (decision_recorded:<key>); reserved §7A. */
  decision_key?: string;
...
export interface SocialCrisisPayload {
  scenario: {
    ...
    initial_state: {
      ...
      // ─── Contract §3 / §5.1 / §7A (multi-organisation scenarios) ───
      ...
      /** Reserved §7A: plausible executive decisions with SOP obligations. */
      decision_space?: ExecutiveDecision[];
      /** Reserved §7A: who should tell whom. */
...
export function assemblePayload(
  ...
  orgName?: string,
  blueprint?: ScenarioBlueprint | null,
  /** Contract §3 / §5.1 / §7A — multi-organisation additions (all optional). */
  multiOrg?: {
    orgs?: OrgRegistryEntry[];
    countries?: CountryEntry[];
    country?: string;
    stakeholders?: Stakeholder[];
    /** Stakeholder-authored and template injects (already scoped/stamped by the generator). */
  ...
  // Stakeholder-authored injects and §7A templates ride alongside; templates
  ...
  return {
    scenario: {
      ...
      initial_state: {
        ...
        ...(multiOrg?.decision_space ? { decision_space: multiOrg.decision_space } : {}),
        ...(multiOrg?.chain_of_command ? { chain_of_command: multiOrg.chain_of_command } : {}),
      },
    },
    teams,
    objectives,
