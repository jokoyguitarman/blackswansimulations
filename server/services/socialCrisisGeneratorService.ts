...
export interface NPCPersona {
  ...
  faction_id?: string;
  alignment?: string;
  // Contract §5.3: the public sphere this persona belongs to. Ambient engines pick
  // reacting personas from the same country as the post; absent = global.
  country?: string;
}
...
export interface SocialInjectDeliveryConfig {
  ...
  /** Organisation this inject is FOR: email/phone/group_chat delivery scope; informational on feed/news. */
  org_key?: string;
  /** Country whose feed / News this content belongs in; absent = every country. */
  country?: string;
  /** Stable cross-reference for chaining (inject_published:<key> / inject_cancelled:<key>). */
  inject_key?: string;
  /** Second-order chaining: the inject_key this one follows (runtime cascade, organic decisions). */
  parent_inject_key?: string;
  /** Runtime cascade linkage: the detected executive decision this inject belongs to. */
  decision_id?: string;
  /** Page-authored inject (contract v3.2): author fields are the page's identity; stakeholder_id = spokesperson. */
  page_org_key?: string;
  /** Email category used by generation prompts and the contacts fallback. */
  email_category?: string;
  /** Pre-generated media attached at compile / regenerated by the editor. */
  media_urls?: string[];
}
...
export interface SOPStep {
  ...
  /** Cast-generated notification steps: the function graded on them (organic decisions §6.6). */
  owner_function?: string;
  /** 'decision_notification' = clock starts when a decision is detected, not at session start. */
  trigger?: 'decision_notification';
  /** Ordering constraint for the notification SOP (union/representatives before employees, etc.). */
  must_precede?: string[];
}
...
export type OrgRole = 'protagonist' | 'antagonist' | 'pressure';
export type OrgControlMode = 'player' | 'ai' | 'trainer';

/** Voice and goals of an AI-run page (pressure orgs; `aligned` for AI-operated protagonist offices). */
export interface PagePosture {
  register: 'statutory' | 'advocacy' | 'grassroots' | 'political' | 'aligned';
  /** What this body exists to do (1 sentence). */
  mandate: string;
  /** 2–4 concrete demands on the protagonist(s); empty for `aligned`. */
  demands: string[];
  /** Ordered rungs: statement → demand with deadline → action (inspection / notice / boycott). */
  escalation_ladder: string[];
  /** Protagonist org_keys this body is pressing (or, for `aligned`, the HQ it follows). */
  targets_org_keys: string[];
  /** What visibly satisfies them; mirrors the spokesperson's resolution_criteria. */
...
export interface OrgConfig {
  ...
  display_name: string;
  is_primary: boolean;
  /** protagonist = player-assignable; antagonist = AI-driven rival; pressure = AI-driven union/regulator/NGO/… */
  role: OrgRole;
  /** who drives the page at runtime. Antagonists and pressure pages default to 'ai'. */
  ...
  /** Pressure pages and AI-operated offices (contract v3.2). */
  kind?: string;
  posture?: PagePosture;
  /** Contactable person who speaks for a pressure page (Stakeholder.id). */
  spokesperson_stakeholder_id?: string;
  /** Protagonist orgs: 'ai' when no humans play this office (page run by the pressure engine, aligned). */
  ...
  /** Contract §5: where this page's organisation sits. Must agree with initial_state.orgs[]. */
  country?: string;
  city?: string;
}
...
export function normalizeOrgPages(
  ...
  if (Array.isArray(op.orgs) && op.orgs.length > 0) {
    orgs = op.orgs.map((o, i) => {
      const role: OrgRole =
        o.role === 'antagonist' ? 'antagonist' : o.role === 'pressure' ? 'pressure' : 'protagonist';
      // Pressure pages and AI-operated offices are AI-driven unless the trainer seized them.
      const aiByDefault = role === 'pressure' || o.operation === 'ai';
      const control_mode: OrgControlMode =
        o.control_mode === 'ai' || o.control_mode === 'trainer'
          ? o.control_mode
          : aiByDefault
            ? 'ai'
            : 'player';
...
export interface SocialCrisisPayload {
  scenario: {
    ...
    initial_state: {
      ...
      // ─── Contract §3 / §5.1 (multi-organisation scenarios) ───
      /** Primary organisation's country (legacy scalar; extremist hive reads it). */
      country?: string;
      /** Canonical organisation registry. */
      orgs?: OrgRegistryEntry[];
      /** {name, code} per distinct country in the registry. */
      countries?: CountryEntry[];
      /** Developed stakeholder characters (contacts workbook, reply engines, reconsideration). */
      stakeholders?: Stakeholder[];
      /**
       * Private hints for the organic executive-decision planner
       * (docs/executive-decisions-organic-plan.md §4.1 / §6.2). Not contract surface.
       */
      decision_context?: {
        leakiness?: number;
...
/** Exported for sibling generator files (stakeholders, decision layer) — same model, same JSON mode. */
export const callSocialCrisisAI = callAI;

/**
 * Organisation context threaded into per-org prompts in multi-organisation
 * scenarios (contract §5). Absent = single-org behaviour, byte-identical prompts.
 */
export interface OrgPromptContext {
  display_name: string;
  short_name: string;
  country: string;
  city?: string;
  kind: string;
  /** Other organisations in the scenario, flagged same-country vs other-country. */
  other_orgs: Array<{ display_name: string; country: string; same_country: boolean }>;
}

function orgContextBlock(org?: OrgPromptContext | null): string {
  if (!org) return '';
  const others =
    org.other_orgs.length > 0
      ? `\nOTHER ORGANISATIONS IN THIS EXERCISE (they run their own teams; coordinate with them, do not act for them):\n${org.other_orgs
          .map(
            (o) =>
              `- ${o.display_name} (${o.country}${o.same_country ? ', same country' : ', other country'})`,
          )
          .join('\n')}`
      : '';
  return `\nORGANISATION: ${org.display_name} (${org.short_name}) — ${org.kind}, ${org.city ? `${org.city}, ` : ''}${org.country}. All content for this organisation is set in ${org.country}: local institutions, regulators, media, place names, currency and register.${others}\n`;
}
...
{ "teams": [{ "team_name": "${names.join('|')}", "mission": "...", "responsibilities": ["..."] }] }`,
    `Crisis: ${crisisType}${orgNameLine(orgName)}${orgContextBlock(org)}\nCountry: ${country}\nContext: ${context}`,
    6000,
  );
...
Return ONLY valid JSON with exactly those keys.`,
      `Crisis: ${crisisContext.crisisType}${orgNameLine(crisisContext.orgName)}${orgContextBlock(org)}\nCountry: ${crisisContext.country}\nContext: ${crisisContext.context}`,
      4000,
    );
...
export async function generateAllTeamStorylines(
  ...
  await Promise.all(
    teams.map(async (team) => {
      const injects = await generateTeamStoryline(team, crisisContext, npcs, factSheet, teams, org);
...
export async function generateUnifiedStoryline(
  ...
  teams?: TeamDef[],
  /** Multi-organisation scenarios: orgs by country so the backbone spans the whole footprint. */
  orgs?: Array<{ display_name: string; country: string; team_names: string[] }>,
): Promise<SocialInject[]> {
  const npcContext = npcs
    .map(
      (p) =>
...
ORGANISATIONS AND COUNTRIES (this crisis spans several organisations; the public conversation happens per country):
${orgs.map((o) => `- ${o.display_name} (${o.country}): teams ${o.team_names.join(', ')}`).join('\n')}
Spread the storyline across the countries involved: events in one country are picked up in the others (media pickups, official reactions, public comparisons). Use personas from the matching country for each post.`
      : '';
...
export async function generateIntelDependencies(
  ...
  /** Multi-organisation scenarios: teams grouped by org so dependencies cross agencies and borders. */
  orgs?: Array<{ org_key: string; display_name: string; country: string; team_names: string[] }>,
  ...
  const multiOrg = !!orgs && orgs.length > 1;
  const orgByTeam = new Map<string, { org_key: string; display_name: string; country: string }>();
  for (const o of orgs || []) for (const t of o.team_names) orgByTeam.set(t, o);
  ...
  const npcHandles = npcs
    ...
    .slice(0, multiOrg ? 16 : 8)
    .map((p) => `${p.handle} (${p.name}, ${p.type}${p.country ? `, ${p.country}` : ''})`)
    .join(', ');

  // Coverage rule for multi-org: every ordered pair of organisations gets at least one direction
  // (same-country pairs = inter-agency silos; cross-country pairs = cross-border coordination).
  const orgPairs: string[] = [];
  if (multiOrg) {
    for (const a of orgs!)
      for (const b of orgs!) {
        if (a.org_key === b.org_key) continue;
        orgPairs.push(
          `${a.display_name} -> ${b.display_name} (${a.country === b.country ? 'same country' : 'cross-border'})`,
        );
      }
  }
  const itemCount = multiOrg
    ? Math.min(8, Math.max(4, orgs!.length * 2))
    : Math.min(teamNames.length, 4);
  const orgBlock = multiOrg
    ? `\n\nORGANISATIONS AND THEIR TEAMS (dependencies MUST cross organisation lines; a fact one agency's inbox receives that another agency needs):\n${orgs!
        .map((o) => `- ${o.display_name} (${o.country}): ${o.team_names.join(', ')}`)
        .join(
          '\n',
        )}\nCover as many of these directions as possible, at least one per pair when the item count allows: ${orgPairs.join('; ')}. Cross-border items involve facts only visible from one country (departure records, local sightings, a local regulator's notice). Gate posts must be authored by personas from the NEEDED-BY organisation's country.`
    : '';

  const result = await callAI(
    `You are designing CROSS-TEAM INTEL DEPENDENCIES for a social media crisis simulation with these teams: ${teamNames.join(', ')}.${orgBlock}
...
Create ${itemCount} intel items — ${multiOrg ? 'holders spread across organisations' : 'at most one held by each team'}, each needed by a DIFFERENT team${multiOrg ? ' in a DIFFERENT organisation' : ''}. For each item provide:
...
{ "intel_items": [{ "intel_key": "...", "holder_team": "...", "needed_by": ["..."], "trigger_time_minutes": 8, "deadline_minutes": 25, "from_name": "...", "from_address": "...", "priority": "high", "email_title": "...", "email_content": "Subject: ...\\n...", "intel_summary": "...", "detection_keywords": ["..."], "positive_gate": { "title": "...", "content": "...", "author_handle": "@npc", "author_display_name": "...", "author_type": "npc_media", "platform": "x_twitter" }, "negative_gate": { "title": "...", "content": "...", "author_handle": "@npc", "author_display_name": "...", "author_type": "npc_media", "platform": "x_twitter" } }] }`,
  ...
  for (const item of rawItems) {
    ...
    // Scoping (code, not model): the email is private to the holder's org; the gates surface in the
    // NEEDED-BY organisation's country feed (that is where the blind spot becomes public).
    const holderOrg = orgByTeam.get(holder);
    const neededOrg = orgByTeam.get(neededBy[0]);
    const emailScope =
      multiOrg && holderOrg ? { org_key: holderOrg.org_key, country: holderOrg.country } : {};
    const gateScope =
      multiOrg && neededOrg ? { org_key: neededOrg.org_key, country: neededOrg.country } : {};
    ...
    if (multiOrg) {
      logger.info(
        {
          holderOrg: holderOrg?.org_key,
          neededByOrgs: neededBy.map((t) => orgByTeam.get(t)?.org_key),
          intelKey: key,
        },
...
export interface OrgRosterEntry {
  ...
  x_handle?: string;
  /** Contract §5: per-organisation location; defaults to the primary's country. */
  country?: string;
  city?: string;
  /** Registry key to use for this page (keeps org_page.orgs[].org_key === initial_state.orgs[].org_key). */
  org_key?: string;
  kind?: string;
}
...
export async function generateOrgPageConfig(
  ...
  /** Contract §5: primary organisation's location stamped onto its page. */
  primaryMeta?: { country?: string; city?: string },
...
Return ONLY valid JSON:
{
  ...
  const primaryOrg: OrgConfig = {
    ...
    country: primaryMeta?.country || country,
    ...(primaryMeta?.city ? { city: primaryMeta.city } : {}),
  };
...
Where an organization lists its own country, make its identity, bio, and branded history authentic to THAT country (institutions, place names, register), not to the primary's.
...
{ "orgs": [{ "org_role": "antagonist", "display_name": "...", "facebook": { "page_name": "...", "page_handle": "@...", "page_bio": "...", "follower_count": 40000 }, "x_twitter": { "page_name": "...", "page_handle": "@...", "page_bio": "...", "follower_count": 25000 }, "stance": "...", "auto_generated": false, "branded_history": [{ "content": "...", "platform": "facebook", "post_format": "text", "days_ago": 7, "media_description": "" }] }] }`,
  ...
  for (let i = 0; i < rawOrgs.length; i++) {
    ...
    orgs.push({
      // Registry-provided key wins so org_page.orgs[].org_key === initial_state.orgs[].org_key.
      org_key:
        requested?.org_key ||
        `org_${role}_${displayName.toLowerCase().replace(/[^\w]/g, '_')}_${i}`,
      ...
      ...(requested?.city ? { city: requested.city } : {}),
    });
  }
...
export function assemblePayload(
  ...
  orgName?: string,
  blueprint?: ScenarioBlueprint | null,
  /** Contract §3 / §5.1 — multi-organisation additions (all optional). */
  multiOrg?: {
    orgs?: OrgRegistryEntry[];
    countries?: CountryEntry[];
    country?: string;
    stakeholders?: Stakeholder[];
    /** Stakeholder- and page-authored injects (already scoped/stamped by the generator). */
    extraInjects?: SocialInject[];
    /** Private planner hints (organic executive decisions). */
    decision_context?: SocialCrisisPayload['scenario']['initial_state']['decision_context'];
  },
): SocialCrisisPayload {
  ...
  // Stakeholder- and page-authored injects ride alongside; condition-gated ones
  // (trigger null + conditions) fall into the condition bucket below.
  const extra = multiOrg?.extraInjects || [];
  ...
  const timeInjects = [
    ...
    ...extra.filter((i) => i.trigger_time_minutes != null && !i.trigger_condition),
    ...sharedInjects,
  ];
  const conditionInjects = [
    ...convergenceGates,
    ...strategyInjects,
    ...extra.filter((i) => i.trigger_time_minutes == null && !i.trigger_condition),
  ];
  const decisionInjects = [
    ...allStoryInjects.filter((i) => i.trigger_condition),
    ...extra.filter((i) => i.trigger_condition),
  ];
  ...
  return {
    scenario: {
      ...
      initial_state: {
        ...
        ...(blueprint ? { blueprint } : {}),
        ...(multiOrg?.country ? { country: multiOrg.country } : {}),
        ...(multiOrg?.orgs ? { orgs: multiOrg.orgs } : {}),
        ...(multiOrg?.countries ? { countries: multiOrg.countries } : {}),
        ...(multiOrg?.stakeholders ? { stakeholders: multiOrg.stakeholders } : {}),
        ...(multiOrg?.decision_context ? { decision_context: multiOrg.decision_context } : {}),
      },
    },
    teams,
    objectives,
