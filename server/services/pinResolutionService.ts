...
/**
 * Evaluate casualty pins — advance lifecycle based on nearby assets.
 */
export async function evaluateCasualtyResolution(sessionId: string): Promise<void> {
  ...
  for (const cas of casualties) {
    ...
    const hasMedical = nearbyAssets.some(
      (a) =>
        a.asset_type.toLowerCase().includes('medic') ||
        a.asset_type.toLowerCase().includes('paramedic') ||
        a.asset_type.toLowerCase().includes('emt'),
    );
    ...
    if (newStatus && newStatus !== cas.status) {
      ...
      try {
        getWebSocketService().broadcastToSession(sessionId, {
          type: 'casualty.updated',
          data: { casualty_id: cas.id, status: newStatus },
          timestamp: new Date().toISOString(),
        });
      } catch {
        /* ws not initialized */
